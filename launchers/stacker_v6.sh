...
#!/usr/bin/env bash
# Spacepresso — XGBoost stacker v6, 11-method bundle (dnv2b14r retrain).
...
# Sanity check
for D in "$EXP2" "$EXP3" "$EXP4" "$EXP5" "$EXP6" \
...
# ─────────────────────────────────────────────────────────────────────────────
echo "RUN V6-A — v6 defaults, no tuning, no calibration"
OUT_A=$RUNS_DIR/${TS}_stacker_xgb_v700
uv run python "$STACKER" \
    "${COMMON_ARGS[@]}" \
    --rank-norm per-class \
    --tune-mode none \
    --calibrate none \
    --out "$OUT_A/submission.csv" \
    --run-tag "stacker-xgb-v700"

# ─────────────────────────────────────────────────────────────────────────────
# RUN V6-B — v5 fixes + per-class Optuna under the pooled CV objective.
# ─────────────────────────────────────────────────────────────────────────────
echo "RUN V6-B — v5 fixes + per-class tuning"
OUT_B=$RUNS_DIR/${TS}_stacker_xgb_v6_B
uv run python "$STACKER" \
