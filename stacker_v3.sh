...
DATA=$ROOT/data
STACKER=$ROOT/xgboost_stacker_v3.py
...
## ─────────────────────────────────────────────────────────────────────────────
#echo "RUN V3-C — v3 fixes + tuning + isotonic"
#OUT_C=$RUNS_DIR/${TS}_stacker_xgb_v3_C
#uv run python "$STACKER" \
#    "${COMMON_ARGS[@]}" \
#    --rank-norm per-class \
#    --tune-mode per-class \
...
#    --run-tag "stacker-xgb-v3-C"

## ─────────────────────────────────────────────────────────────────────────────
## RUN V3-D — ABLATION: v3 with global rank-norm (i.e. only the pooled-CV +
## small-CC + consensus + CVD changes; the rank-norm change is reverted).
## Useful to isolate how much of v3's gain comes from per-class rank-norm.
## ─────────────────────────────────────────────────────────────────────────────
#echo "RUN V3-D — ablation: v3 but global rank-norm"
#OUT_D=$RUNS_DIR/${TS}_stacker_xgb_v3_D
#uv run python "$STACKER" \
#    "${COMMON_ARGS[@]}" \
#    --rank-norm global \
#    --tune-mode none \
...
#    --run-tag "stacker-xgb-v3-D-ablation"
