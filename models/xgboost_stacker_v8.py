...
# ─────────────────────────────────────────────────────────────────────────────
# v7.4 — CRITICAL bug fix shadowing v6.fit_calibrator
# ─────────────────────────────────────────────────────────────────────────────
def fit_calibrator(method, oof_preds, oof_labels) -> dict:
    """Replaces v6's fit_calibrator with a stratified-downsample variant.

    The v6 bug: for isotonic, it did
        target_neg = max(n_max - n_pos, 0)
    which produces target_neg=0 whenever n_pos > n_max. The downsampled
    set then contained ZERO negatives, isotonic fit a constant 1.0
    everywhere, and every test pixel of that class mapped to the same
    score. After the global rank-norm those tied predictions became
    arbitrary ranks within the class → per-class AP collapsed to the
    class's positive base rate.

    Symptoms in the wild: class_06 post-cal AP dropped from 0.88 to 0.25
    (= 25% positive rate), class_08 from 0.97 to 0.21 (= 21% positive
    rate). Other classes were fine because they had n_pos < n_max.

    Fix: cap pos AND neg independently at n_max // 2 each. Both classes
    are represented even when one of them dominates the OOF.

    Behaviour is byte-identical to v6 for 'none' and 'platt'. For
    'isotonic' it differs only in the n_pos > n_max // 2 regime —
    classes with fewer positives produce identical fits.
    """
    if method == "none":
        return {"method": "none"}
    ...
    if method == "isotonic":
        ...
        n_max = 1_000_000
        n_pos = int((oof_labels == 1).sum())
        n_neg = int((oof_labels == 0).sum())
        if oof_preds.size > n_max:
            # v7.4: stratified cap. Each class gets at most n_max // 2.
            target_pos = min(n_pos, n_max // 2)
            target_neg = min(n_neg, n_max // 2)
            pos_idx = np.flatnonzero(oof_labels == 1)
            neg_idx = np.flatnonzero(oof_labels == 0)
            rng = np.random.default_rng(0)
            spos = (rng.choice(pos_idx, size=target_pos, replace=False)
                    if target_pos < pos_idx.size else pos_idx)
            sneg = (rng.choice(neg_idx, size=target_neg, replace=False)
                    if target_neg < neg_idx.size else neg_idx)
            sel = np.concatenate([spos, sneg])
            oof_preds = oof_preds[sel]
            oof_labels = oof_labels[sel]
            print(f"    [calib] n_pos={n_pos:>8d} n_neg={n_neg:>8d} → "
                  f"sampled pos={target_pos:>7d} neg={target_neg:>7d}",
                  flush=True)
        ir = IsotonicRegression(out_of_bounds="clip", y_min=0.0, y_max=1.0)
        ir.fit(oof_preds, oof_labels.astype(np.float32))
        # Defensive sanity check: refuse to return a degenerate constant.
        # (Shouldn't trigger with the stratified fix, but is cheap insurance
        # against future regressions / edge cases.)
        ys = np.asarray(ir.y_thresholds_, dtype=np.float64)
        if ys.size >= 2 and float(ys.max() - ys.min()) < 1e-9:
            print(f"    [calib] WARN: isotonic collapsed to a constant "
                  f"({float(ys.mean()):.4f}) — falling back to no-op for "
                  f"this class to avoid destroying its ranking.")
            return {"method": "none"}
...
def configure_xgb_device(mode: str = "auto") -> bool:
    ...
    # Also tune n_jobs on GPU — the GPU does the heavy lifting; using
    # all CPU cores for DMatrix prep contends with itself across many
    # small fold fits and slows things down, but using too FEW starves
    # the GPU. 8 is a good default on modern workstations.
    _v6.DEFAULT_XGB_PARAMS["n_jobs"] = 8
    return True
...
def free_xfull_post_oof(training_data: dict) -> None:
    """v7.5: Drop X_full/y_full/buckets from each class once OOF is done.

    Why this matters (the bug that caused the OOM at end of fit_per_class):
        After build_training_data, each class holds BOTH X_full and
        X_train as independent ndarrays (X_train = X_full[keep] is a
        numpy fancy-index copy, not a view). X_full is only used by the
        OOF k-fold path; fit_per_class only reads X_train.

        Without this free, the production fit carries:
            training_data with X_full + X_train  ≈ 30-50 GB
          + SHARED model concat (np.concatenate of all X_trains in v6's
            fit_per_class)                        ≈ 16 GB
          → 50-65 GB peak, which OOMs on a 56 GB host.

        Dropping X_full early brings the production fit's working set
        down to X_train + SHARED concat ≈ 32 GB, well under budget.

    Also drops img_pixranges and anomaly_types: those are k-fold bucketing
    metadata used by loao_oof_one_class_fp16safe, never by fit_per_class.

    Call this AFTER OOF + calibration + verification, BEFORE fit_per_class.
    """
    n_before = mem_rss_gb()
    n_freed = 0
    bytes_freed = 0
    for cls in list(training_data.keys()):
        ...
        if not isinstance(td, dict): continue
        if td.get("_fallback_to_shared"): continue
        for key in ("X_full", "y_full", "img_pixranges", "anomaly_types"):
            if key in td:
                arr = td.pop(key)
                if hasattr(arr, "nbytes"):
                    bytes_freed += arr.nbytes
                del arr
        n_freed += 1
    gc.collect()
    n_after = mem_rss_gb()
    print(f"  [free_xfull_post_oof] dropped X_full/y_full/buckets from "
          f"{n_freed} classes  (~{bytes_freed / 1e9:.1f} GB freed)  "
          f"RSS {n_before:.2f} → {n_after:.2f} GB")


def fit_per_class_v7_no_shared(training_data: dict,
                                    params_global: dict,
                                    params_per_class: dict | None,
                                    seed: int,
                                    *,
                                    min_pos_for_per_class: int = 200,
                                    drop_xtrain_after_fit: bool = True
                                    ) -> dict:
    """v7.6: Skip the never-used SHARED-fallback model and free each
    class's X_train as soon as that class is fit.

    Background — what v6's fit_per_class does:
        It builds, per class, a "SHARED" fallback by
            X_all = np.concatenate([td["X_train"] for cls in classes])
        and fits one global XGB on it. This SHARED model only gets
        used as a fallback for tiny classes (n_pos < 200) that didn't
        get their own model. On this dataset NO class has n_pos < 200,
        so SHARED is fit, allocates ~16 GB transient, takes ~30s, and
        is then never consulted.

        On a 56 GB host, with training_data still holding all X_train
        arrays (~16 GB), the SHARED concat brings peak RAM to ~32 GB
        on top of whatever else is live (val gone, OOF stored). It's
        the alloc that pushed the user over the limit.

    What this function does instead:
        1) Detect whether every class has n_pos >= min_pos_for_per_class.
           - If YES: skip SHARED entirely. Fit each class on its own,
             and (if drop_xtrain_after_fit=True) free that class's
             X_train/y_train immediately after the fit completes. Peak
             RAM during fit_per_class drops to ~(remaining X_trains +
             current class fit overhead), bounded by the LARGEST single
             X_train — about a 5× reduction in peak.
           - If NO: defer to v6.fit_per_class so the fallback path
             still works (we don't silently change behaviour on
             datasets that actually need SHARED).

        2) Keep the same return contract as v6.fit_per_class — a dict
           `{class_name: {"model": clf, "n_pos": ..., "params": ...,
           "n_features": ...}, "_feature_names": [...]}`. fuse_test_v7
           doesn't care that "_SHARED_" is absent because it only looks
           it up when a class entry is missing or marked fallback.

    Memory math for this dataset:
        Before:  ~16 GB (all X_train) + 16 GB SHARED concat = 32 GB peak
        After:   ~3 GB (max single X_train) during fit + accumulating
                 models (small) ≈ ~4 GB peak in fit_per_class.
        Net win: ~28 GB.
    """
    ...
    class_list = [cls for cls in training_data.keys()
                   if not cls.startswith("_")
                   and isinstance(training_data[cls], dict)
                   and not training_data[cls].get("_fallback_to_shared")]

    needs_shared = [cls for cls in class_list
                     if training_data[cls].get("n_pos", 0)
                          < min_pos_for_per_class]

    if needs_shared:
        print(f"  [v7.6] {len(needs_shared)} class(es) below "
              f"min_pos_for_per_class={min_pos_for_per_class}: "
              f"{needs_shared}")
        print(f"  [v7.6] Falling back to v6.fit_per_class so SHARED is "
              f"available.")
        return _v6.fit_per_class(training_data, params_global,
                                       params_per_class, seed,
                                       min_pos_for_per_class=min_pos_for_per_class)

    print(f"  [v7.6] All {len(class_list)} classes have "
          f"n_pos >= {min_pos_for_per_class}.")
    print(f"  [v7.6] Skipping the never-used SHARED-fallback model "
          f"(saves ~16 GB transient concat + ~30s fit).")
    if drop_xtrain_after_fit:
        print(f"  [v7.6] Will drop each class's X_train/y_train "
              f"immediately after its fit (peak RAM during "
              f"fit_per_class ≈ largest single X_train, not Σ).")

    rss_pre = mem_rss_gb()
    print(f"  [v7.6] entering loop  RSS={rss_pre:.2f} GB", flush=True)

    for i, cls in enumerate(class_list):
        td = training_data[cls]
        params = get_params_for_class(cls, params_global, params_per_class)
        n_neg = int((td["y_train"] == 0).sum())
        n_pos = int(td["n_pos"])
        nf = int(td["X_train"].shape[1])

        clf = _make_xgb(params, seed)
        t0 = time.time()
        clf.fit(td["X_train"], td["y_train"])
        elapsed = time.time() - t0

        # Compute train metrics (matches v6 output for log parity).
        train_preds = clf.predict_proba(td["X_train"])[:, 1]
        train_preds_clipped = np.clip(train_preds, 1e-7, 1 - 1e-7)
        try:    tll = float(log_loss(td["y_train"], train_preds_clipped))
        except Exception: tll = float("nan")
        try:    tap = float(average_precision_score(td["y_train"], train_preds))
        except Exception: tap = float("nan")

        print(f"  class {cls}: n_pos={n_pos:>7d} n_neg={n_neg:>7d}  "
              f"train_logloss={tll:.4f}  train_ap={tap:.3f}  "
              f"({elapsed:.1f}s)", flush=True)

        out[cls] = {"model": clf, "n_pos": n_pos,
                     "params": params, "n_features": nf}

        # Local cleanup of metric buffers BEFORE we touch td["X_train"]
        del train_preds, train_preds_clipped

        if drop_xtrain_after_fit:
            # Drop this class's X_train/y_train. No more references
            # exist after this — fit_per_class doesn't aggregate.
            ...
            gc.collect()
            rss_now = mem_rss_gb()
            print(f"    [v7.6] dropped X_train/y_train for {cls}  "
                  f"RSS={rss_now:.2f} GB  ({i + 1}/{len(class_list)} done)",
                  flush=True)

    rss_post = mem_rss_gb()
    print(f"  [v7.6] fit_per_class done.  RSS {rss_pre:.2f} → "
          f"{rss_post:.2f} GB  ({len(class_list)} models fit, "
          f"SHARED skipped).")
    return out
...
def tune_global_pooled(training_data, *, n_trials, seed,
    ...
    # one-line summary in the callback and a brief 'starting' heartbeat.
    state = {"trial_idx": 0, "study_t0": time.time()}

    def objective(trial):
        params = _optuna_suggest(trial)
        verbose = (state["trial_idx"] == 0)
        t0 = time.time()
        # Visible heartbeat at the start of EVERY trial so the user knows
        # the study is alive even when per-class progress is suppressed.
        elapsed_study = (time.time() - state["study_t0"]
                          if state["study_t0"] is not None else 0.0)
        print(f"\n  [trial {trial.number:>3d}/{n_trials}] starting "
              f"(elapsed study={elapsed_study:.0f}s)",
              flush=True)
        if verbose:
            print(f"        (verbose per-class for first trial only)",
                  flush=True)
    ...
    print(f"")
    print(f"    EARLY STOP (current trial finishes, then continue with "
          f"best-so-far):")
    print(f"        attached terminal:    Ctrl+C  (once)")
    print(f"        backgrounded job:     kill -USR1 {os.getpid()}")
    print(f"    HARD STOP (abort even the current trial):")
    print(f"        attached terminal:    Ctrl+C  (twice)")
    print(f"        backgrounded job:     kill -INT  {os.getpid()}  (twice)")
    print(f"")

    # Graceful early-stop: first SIGINT calls study.stop() (Optuna stops
    # accepting new trials but the current one still runs to completion).
    # Second SIGINT restores Python's default handler which raises
    # KeyboardInterrupt — fastest path to a hard kill.
    sig_state = {"count": 0}
    ...
    def _sigint_handler(signum, frame):
        sig_state["count"] += 1
        if sig_state["count"] == 1:
            print(f"\n  [signal] SIGINT received — telling Optuna to "
                  f"stop after the current trial finishes. "
                  f"Press Ctrl+C again to hard-quit.", flush=True)
            try:
                study.stop()
            except Exception as e:
                print(f"  [signal] study.stop() raised: {e}", flush=True)
        else:
            print(f"\n  [signal] SIGINT (2nd) — restoring default "
                  f"handler; next Ctrl+C will hard-quit.", flush=True)
            signal.signal(signal.SIGINT, prev_sigint)

    def _sigusr1_handler(signum, frame):
        # SIGUSR1 is "soft stop" only — never escalates to a hard kill.
        # Use SIGINT/SIGTERM if you want to abort outright.
        print(f"\n  [signal] SIGUSR1 received — telling Optuna to "
              f"stop after the current trial finishes. "
              f"(Send again for no extra effect; this signal is "
              f"graceful-only.)", flush=True)
        try:
            study.stop()
        except Exception as e:
            print(f"  [signal] study.stop() raised: {e}", flush=True)
    ...
    try:
        prev_sigusr1 = signal.signal(signal.SIGUSR1, _sigusr1_handler)
    except (AttributeError, ValueError, OSError):
        # SIGUSR1 unavailable (Windows, or not main thread). Skip silently.
        pass
    ...
    try:
        ...
        # Second Ctrl+C path. If we got here, at least let the user know
        # what state we're in before re-raising.
        completed = [t for t in study.trials
                      if t.state.name == "COMPLETE"]
        print(f"\n  [signal] hard-quit during tuning. "
              f"{len(completed)} completed trial(s); "
              f"will use best of those.", flush=True)
    finally:
        signal.signal(signal.SIGINT, prev_sigint)
    ...
    # Defensive: if every trial failed (zero completed), there's nothing
    # to proceed with. Tell the caller loudly.
    completed = [t for t in study.trials if t.state.name == "COMPLETE"]
    if not completed:
        raise SystemExit(
            "[FATAL] no trials completed; cannot continue. Either the "
            "study was cancelled before trial 0 finished, or every "
            "trial errored. Re-run with --tune-mode none + explicit "
            "XGB params if you have known-good values.")
    print(f"\n>>> best global pooled-AP (OOF): {study.best_value:.4f}")
    print(f">>> best params: {json.dumps(study.best_params, indent=2)}")
    print(f">>> completed {len(completed)}/{len(study.trials)} trial(s) "
          f"(remaining were interrupted or pruned)")
...
def main():
    ...
    ap.add_argument("--xgb-n-jobs", type=int, default=None,
                    help="Override XGBoost n_jobs. Defaults: -1 on CPU, "
                         "8 on GPU. Raise to 16+ if GPU is starved by "
                         "CPU-side DMatrix construction (visible as low "
                         "GPU utilisation and slow per-fold timings).")
    ...
    with tee_to(run_dir / "run_log.txt"):
        ...
        # CLI override for n_jobs takes precedence over the device default
        if args.xgb_n_jobs is not None:
            _v6.DEFAULT_XGB_PARAMS["n_jobs"] = int(args.xgb_n_jobs)
            print(f"  [xgb] n_jobs override: {args.xgb_n_jobs}")
        ...
        # v7.5: X_full is no longer needed once OOF + calibration + verify
        # are done. Free it BEFORE fit_per_class to avoid the OOM caused
        # by the SHARED-model np.concatenate sitting on top of X_full.
        print(f"\n[v7.5] Freeing X_full before production fit...")
        free_xfull_post_oof(training_data)

        print(f"\nFitting final per-class XGB models...")
        models = fit_per_class_v7_no_shared(training_data, params_global,
