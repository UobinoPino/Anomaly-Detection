# """Spacepresso UniAD baseline.
#
# Implements "A Unified Model for Multi-class Anomaly Detection" (You et
# al., NeurIPS 2022), adapted to per-class training to match the rest of
# the Spacepresso codebase. Despite the name "unified", we train one model
# per class -- the architectural innovations (neighbour-masked attention,
# layer-wise query decoder, feature jittering) still help in single-class
# mode and give the stacker a genuinely different signal.
#
# # Architecture
#
#   TEACHER (frozen, shared across classes):
#     EfficientNet-B4 (ImageNet). Multi-stage features fused at H/16:
#       block 2 [B,  32, H/4,  W/4]   downsampled to H/16
#       block 3 [B,  56, H/8,  W/8]   downsampled to H/16
#       block 5 [B, 160, H/16, W/16]  identity
#       block 6 [B, 272, H/32, W/32]  upsampled to H/16
#     Concatenated -> (B, 520, H/16, W/16).
#     At input 256 this is 16x16 = 256 tokens; at input 384 it's 24x24
#     = 576 tokens.
#
#   TOKENISATION:
#     Flatten spatial to tokens of length L = (H/16) * (W/16).
#     Add a learnable positional embedding.
#
#   ENCODER:
#     N_enc transformer blocks with neighbour-masked self-attention. Each
#     token can only attend to tokens whose spatial Chebyshev distance is
#     NOT in the masked set (default: mask exact-position to prevent the
#     trivial identity shortcut). This is the core anti-shortcut device of
#     UniAD: the model must reconstruct each token from its neighbours,
#     not from itself.
#
#   LAYER-WISE QUERY DECODER:
#     Learnable queries are added to the encoder output at every decoder
#     block ("layer-wise" injection). Each decoder block does:
#         x = x + cross_attn(queries, encoder_out)
#         x = x + self_attn(x)
#         x = x + ffn(x)
#
#   RECONSTRUCTION HEAD:
#     Linear back to teacher dim. Output: (B, 272, H/16, W/16).
#
#   FEATURE JITTERING (train only):
#     Add Gaussian noise to teacher features before feeding the encoder.
#     Forces the model to learn the data manifold rather than overfitting
#     to exact teacher values. Default sigma=20 (relative to feature std).
#
# # Loss (train only on train_good)
#
#     L = MSE( decoder_output, teacher_features )  -- no jittering on target
#
# # Anomaly map at inference
#
#     diff       = (decoder_output - teacher_features) ** 2   # (B, C, h, w)
#     score_lr   = diff.mean(dim=1)                            # (B, h, w)
#     score      = bilinear_upsample(score_lr, (H, W))
#
# # Multi-view inference (--multiview sibling-bank)
#
#   Same idea as EfficientAD: at test time, all 5 views of a sample are
#   batched together. For each view V_i, the 4 sibling views' teacher
#   features form a small memory bank. View-inconsistency at each pixel
#   in V_i (1 - max cosine sim to sibling bank) is normalised per-sample
#   to [0, 1] and multiplied into the reconstruction score:
#       final = recon_score * ((1 - alpha) + alpha * mv_inconsistency)
#
# # Memory / speed (L4 24 GB)
#
#   Per class @ input 256, batch 16, 2500 iters:
#     Training:  ~6 min (transformer with H/16 = 16x16 tokens = 256)
#     Standard:  ~25 s eval + 25 s test
#     Multi-view: ~40 s eval + 40 s test
#   Full 8 classes: ~60 min standard, ~75 min multi-view.
#
# # Dependencies
#
#   patchcore_baseline_v2.py and local_preds_saver.py in same directory.
#   torchvision >= 0.13 for EfficientNet-B4 weights.
# """
...
#
# # EfficientNet-B4 block-output spatial-stride map (torchvision indexing):
# #   block 0: H/2,  48 ch
# #   block 1: H/2,  24 ch
# #   block 2: H/4,  32 ch
# #   block 3: H/8,  56 ch
...
# #   block 6: H/32, 272 ch
# #   block 7: H/32, 448 ch
# # We pick four stages spanning fine-to-coarse semantics and fuse them
# # at H/16 (the target_stride) to give a 16x16 token grid at input 256:
# #   block 2 (H/4,  32) -> downsample to H/16
# #   block 3 (H/8,  56) -> downsample to H/16
# #   block 5 (H/16, 160) -> identity
# #   block 6 (H/32, 272) -> upsample to H/16
# EFF_B4_STAGES   = (2, 3, 5, 6)
# EFF_B4_CHANNELS = (32, 56, 160, 272)
# TEACHER_DIM     = sum(EFF_B4_CHANNELS)          # 520
# TARGET_STRIDE   = 16                             # All features fused at H/16
# TARGET_BLOCK_IDX = 5                             # The H/16 reference stage
#
#
# # ─────────────────────────────────────────────────────────────────────────────
# # Tee logger
...
#
# # ─────────────────────────────────────────────────────────────────────────────
# # Teacher: EfficientNet-B4 multi-scale at H/16
# # ─────────────────────────────────────────────────────────────────────────────
# class EfficientNetB4Teacher(nn.Module):
#     """Frozen EfficientNet-B4. Returns features from 4 stages bilinearly
#     upsampled to H/16 and concatenated channel-wise.
#
#     Different family from RN18 (EfficientAD), WRN50 (PatchCore exp5, RD),
#     DINOv2 (exp7) -> intentional diversity for the stacker.
#     """
...
#         # `features` is a Sequential of MBConv stages.
#         self.features = m.features
#         # Verify our stage indices are valid.
#         n_blocks = len(self.features)
#         for s in EFF_B4_STAGES:
#             assert 0 <= s < n_blocks, (
#                 f"EfficientNet-B4 stage {s} out of range [0,{n_blocks-1}]")
...
#             if i >= max(wanted):
#                 break
#         # Identify the H/16 stage and resize everyone to its size.
#         H_target, W_target = outs[TARGET_BLOCK_IDX].shape[-2:]
#         upped = []
#         for s in EFF_B4_STAGES:
#             f = outs[s]
#             if f.shape[-2:] != (H_target, W_target):
#                 f = F.interpolate(f, size=(H_target, W_target),
#                                   mode="bilinear", align_corners=False)
#             upped.append(f)
#         return torch.cat(upped, dim=1)        # (B, TEACHER_DIM, H/16, W/16)
#
#
# # ─────────────────────────────────────────────────────────────────────────────
...
#     Chebyshev (L_inf) distance is <= neighbour_radius. With
#     neighbour_radius=0, only the diagonal (self-attention) is masked --
#     enough to break the trivial identity shortcut. Larger radii mask
#     a square neighbourhood; UniAD paper used radius=7 on 14x14 grids."""
#     rs = torch.arange(h).repeat_interleave(w)   # (L,) row index
#     cs = torch.arange(w).repeat(h)              # (L,) col index
#     dr = (rs.unsqueeze(0) - rs.unsqueeze(1)).abs()
#     dc = (cs.unsqueeze(0) - cs.unsqueeze(1)).abs()
#     cheby = torch.maximum(dr, dc)
#     mask = cheby <= neighbour_radius            # True = forbidden
...
#     """Pre-LN decoder block with cross-attention from queries to encoder
#     output, then self-attention, then FFN. Layer-wise injection means
#     the SAME query embedding is added at every block (handled outside
#     in UniADModel.forward)."""
#     def __init__(self, dim: int, n_heads: int, mlp_ratio: float = 4.0,
#                  dropout: float = 0.1):
#         super().__init__()
...
#         # Cross-attention: queries are x (current decoder state),
#         # keys/values are encoder_out.
#         q = self.ln_cross_q(x)
#         kv = self.ln_cross_kv(encoder_out)
#         h, _ = self.cross_attn(q, kv, kv, need_weights=False)
#         x = x + h
#         # Self-attention on decoder state (with neighbour mask too).
#         h = self.ln_self(x)
...
#     """Per-class UniAD. Constructed knowing the spatial size (h, w) at
#     H/16 so positional embeddings and attention masks can be precomputed."""
...
#
#         # Project teacher features to model_dim.
...
#
#         # Pre-build the neighbour mask. Registered as a buffer so it
#         # moves with .to(device) and is saved/loaded in state_dict.
...
#         """t_feat: (B, C_teacher, h, w). Returns reconstruction in the
#         same shape. jitter_sigma > 0 adds Gaussian noise to the INPUT
#         (only) before the encoder, acting as a regulariser. The target
#         of the loss is the un-jittered teacher feature."""
...
#             # Per-feature std proxy: use the batch std along (B, L) per
#             # channel. Scale jitter by it so we noise small and large
...
#
#         # Encoder with neighbour mask.
#         attn_mask = self.attn_forbid_mask
#         for blk in self.encoder:
#             x = blk(x, attn_mask=attn_mask)
#
#         # Decoder with layer-wise query injection.
#         dec_in = self.query_embed.expand(B, -1, -1).clone()
#         for blk in self.decoder:
#             dec_in = dec_in + self.query_embed   # layer-wise injection
#             dec_in = blk(dec_in, encoder_out=x, self_attn_mask=attn_mask)
#         dec_in = self.dec_ln_out(dec_in)
#         recon = self.output_proj(dec_in)                         # (B, L, C)
#         return recon.reshape(B, h, w, C).permute(0, 3, 1, 2)     # (B, C, h, w)
#
#
# # ─────────────────────────────────────────────────────────────────────────────
# # Training
# # ─────────────────────────────────────────────────────────────────────────────
# def train_uniad(teacher: EfficientNetB4Teacher,
#                  model: UniADModel,
...
#
# # ─────────────────────────────────────────────────────────────────────────────
# # Normalisation stats over train_good (same idea as EfficientAD)
# # ─────────────────────────────────────────────────────────────────────────────
...
# # ─────────────────────────────────────────────────────────────────────────────
# @torch.inference_mode()
# def _compute_maps_one(teacher, model, x: torch.Tensor, stats: dict,
#                        cfg: "RunConfig"
...
#         t_feat = teacher(x)
#         recon = model(t_feat, jitter_sigma=0.0)
#     diff = ((recon.float() - t_feat.float()) ** 2).mean(dim=1)   # (B, h, w)
...
#         s2, t2 = _compute_maps_one(teacher, model,
...
#
# @torch.inference_mode()
# def score_batch_standard(teacher, model, x: torch.Tensor, stats: dict,
...
#                                  ) -> torch.Tensor:
#     """All views of one sample batched together. Cosine-distance to
#     sibling views' teacher features boosts standard reconstruction
#     scores. x_sample: (V, 3, H, W). Returns (V, input_size, input_size)."""
...
#
#
# def _score_records_by_sample(teacher, model, records, stats, cfg,
#                               load_masks):
...
#     """Run a dummy image through the teacher to determine the H/16 size."""
...
#     bits = (f"{stamp}_uniad_effb4_in{cfg.input_size}_d{cfg.model_dim}"
#             f"_e{cfg.n_enc_layers}d{cfg.n_dec_layers}"
...
#                     help="Must be multiple of 32 for EfficientNet-B4's "
#                          "H/32 stage to be a clean integer at H/16.")
#     # Architecture
...
#                          "FORBIDDEN to attend (incl. self). 1 = mask "
#                          "self + 8 neighbours (3x3 window).")
#     ap.add_argument("--jitter-sigma", type=float, default=0.0,
#                     help="Std of Gaussian noise added to teacher features "
#                          "at train time (scaled by per-channel feature "
#                          "std). Disabled by default; try 0.2-0.5 if the "
#                          "model overfits.")
#     # Training
#     ap.add_argument("--epochs", type=int, default=200)
#     ap.add_argument("--total-iters", type=int, default=2500,
#                     help="Overrides --epochs (matches budget used by "
#                          "CutPaste / RD / EfficientAD).")
...
#                          "together and use the OTHER 4 views' teacher "
#                          "features as a small memory bank. Boost each "
#                          "view's pixel scores by view-inconsistency.")
...
#         raise SystemExit(f"[FATAL] --input-size must be multiple of 32 "
#                           f"(got {args.input_size}). Try 256 or 384.")
#
#     cfg = RunConfig(
...
#         hr(f"UNIAD — RUN {run_id}", "█")
#         print(f"  data_root        : {cfg.data_root}")
#         print(f"  run_dir          : {run_dir}")
#         print(f"  teacher          : frozen EfficientNet-B4, "
#               f"layers {EFF_B4_STAGES} fused at H/16 -> {TEACHER_DIM} ch")
...
#
#         teacher = EfficientNetB4Teacher().to(device).eval()
#         feature_hw = _infer_feature_hw(teacher, cfg, device)
#         print(f"\n  teacher H/16 feature grid: {feature_hw} "
...
#             "backbone": "UNIAD_EFFB4",
#             "feature_layers": "+".join(str(s) for s in EFF_B4_STAGES),
#             "target_layer": "",
#             "input_size": cfg.input_size,
...
#             "notes": (f"uniad multiview={cfg.multiview} "
