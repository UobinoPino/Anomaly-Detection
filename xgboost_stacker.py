...
# v2.1 changes (this revision)

1. CV metric switched to POOLED pixel-AP within each held fold, matching
   the leaderboard ("Pixel-Level AP across all possible thresholds").
   Previously cv_score_one_class averaged per-image APs, which is
   insensitive to cross-image score-scale drift and over-reported by
   roughly +0.10 mean AP on our val set. Tuning under the correct metric
   will pick more conservative, better-calibrated configurations.

2. OOF predictions are now harvested from per-class tuning *regardless*
   of --calibrate. Previously they were thrown away when calibrate=none,
   forcing a rebuild_oof.py refit. Now postprocess_stacker.py and any
   calibrator retrofit can read oof_predictions.npz directly from any
   --tune-mode per-class run.
"""
